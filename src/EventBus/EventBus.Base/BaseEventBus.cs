...
using EventBus.EventBus.Base.Manager;
using EventBus.Events;
using Microsoft.Extensions.DependencyInjection;
using Newtonsoft.Json;

namespace EventBus.EventBus.Base
{
            ...
            if (SubscriptionManager.HasSubscriptionForEvent(eventName))
            {
                ...
                using (var scope = ServiceProvider.CreateScope())
                {
                    foreach (var subscription in subscriptions)
                    {
                        ...
                        var eventType = SubscriptionManager.GetEventTypeName($"{EventBusConfig.EventNamePrefix}{eventName}{EventBusConfig.EventNameSuffix}");
